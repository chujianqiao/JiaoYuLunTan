...
layui.use(['form', 'admin', 'ax','laydate','upload','formSelects','upload','selectPlus', 'formSelects'], function () {
    ...
    //获取详情信息，填充表单
    var resultId = Feng.getUrlParam("resultId");
    resultId = resultId.toString().split(";");
    var ajax = new $ax(Feng.ctxPath + "/educationResult/detail?resultId=" + resultId[0]);
    var result = ajax.start();
    form.val('resultForm', result.data);
    ...
    form.on('submit(btnSubmit)', function (data) {
        if ($("#reviewMajor option:selected").val() == undefined || $("#reviewMajor option:selected").val() == ""){
            Feng.error("请选择专家后提交分配！")
        } else {
            var ajax = new $ax(Feng.ctxPath + "/educationResult/assignItem", function (data) {
                Feng.success("分配成功！");
                //传给上个页面，刷新table用
