layui.use(['table', 'admin','form', 'ax', 'func'], function () {
    ...
    EducationResult.jumpAssignPage = function (data) {
        func.open({
            title: '分配评审人',
            area: ['350px', '300px'],
            content: Feng.ctxPath + '/educationResult/assign?resultId=' + data.resultId,
            tableId: EducationResult.tableId
        });
    };

    EducationResult.jumpAssignPageBatch = function (data) {
        var checkRows = table.checkStatus(EducationResult.tableId);
        ...
        if (checkRows.data.length === 0) {
            ...
            func.open({
                title: '分配评审人',
                area: ['350px', '300px'],
                content: Feng.ctxPath + '/educationResult/assign?resultId=' + resultIds,
                tableId: EducationResult.tableId
            });
        }
    };
