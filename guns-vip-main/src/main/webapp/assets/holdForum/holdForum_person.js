layui.use(['table', 'admin', 'ax', 'func'], function () {
     ...
     * @param data 点击按钮时候的行数据
     */
    HoldForum.onCancel = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/holdForum/cancel", function (data) {
                Feng.success("取消成功!");
                table.reload(HoldForum.tableId);
            }, function (data) {
     ...
     * @param data 点击按钮时候的行数据
     */
    HoldForum.onEditNew = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/holdForum/editNew", function (data) {
                Feng.success("申请成功!");
                table.reload(HoldForum.tableId);
            }, function (data) {
    ...
    table.on('tool(' + HoldForum.tableId + ')', function (obj) {
        ...
        if (layEvent === 'edit') {
            ...
            HoldForum.onCancel(data);
        } else if (layEvent === 'editNew') {
            HoldForum.onEditNew(data);
